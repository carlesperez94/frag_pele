...
def test_double():
    ...
    mn.main("1w7h_preparation_structure_2w.pdb", "serie_file_carbonil.conf", cpus=4, 
            iterations=2, steps=3, temperature=1500, pele_eq_steps=1, contrl="test.conf", 
            test=True, growing_protocol="AllLinear", start_growing_from=0)
...
def test_HT():
    ...
    if os.path.exists(output):
        shutil.rmtree(output)
    mn.main("1w7h_preparation_structure_2w.pdb", "serie_file.conf", cpus=4, iterations=2, 
            steps=3, pele_eq_steps=1, contrl="test.conf", protocol="HT", mae=True, 
            test=True, growing_protocol="SoftcoreLike", start_growing_from=0)
...
def test_sequential():
    ...
    for output in outputs:
        if os.path.exists(output):
            shutil.rmtree(output)
    mn.main("1w7h_preparation_structure_2w.pdb", "sequential.conf", cpus=4, iterations=1, 
            steps=1, pele_eq_steps=1, contrl="test.conf", test=True, growing_protocol='SpreadHcharge',
            start_growing_from=0)
    assert glob.glob("1w7h_preparation_structure_2w_aminoC1N1/top_result/*BindingEnergy*.pdb") and glob.glob("aminoC1N1_top_aminoC1N1phenylC6C1/top_result/*BindingEnergy*.pdb")
...
def test_cov_aa():
    ...
    mn.main("receptor_145_cys.pdb", "serie_res.conf", cpus=4, iterations=1, steps=2, pele_eq_steps=2, test=True, cov_res="A:145",
            criteria="LocalNonBondingEnergy", start_growing_from=0, contrl="control_cov_test.conf")
...
def test_cov_mod():
    ...
    mn.main("cov_scaffold.pdb", "serie_res_2.conf", cpus=4, iterations=1, steps=4, pele_eq_steps=2, test=False, cov_res="A:145",
            criteria="LocalNonBondingEnergy", start_growing_from=0, rotamers=10, contrl="control_cov_test.conf")
